...
def _artifact_root() -> Path:
    return Path(__file__).resolve().parents[3] / "data" / "workflows"


def _write_artifact(run: WorkflowRun, name: str, value: Any) -> str:
    directory = _artifact_root() / run.id
    directory.mkdir(parents=True, exist_ok=True)
    path = directory / name
    path.write_text(json.dumps(value, indent=2, default=str), encoding="utf-8")
    relative = str(path.relative_to(_artifact_root().parent.parent))
...
def invoke_specialist(agent_id: str, title: str, objective: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    agent = get_model(context["model_policy"])
    prompt = {
        "objective": objective,
        "assignment": title,
        "agent": agent_id,
        ...
        "instruction": "Return assumptions, decisions, risks, evidence, and validation. Do not claim execution without evidence.",
    }
    response = agent.invoke(json.dumps(prompt))
...
def validate_terraform(terraform_root: Path) -> dict[str, Any]:
    tf_files = list(terraform_root.rglob("*.tf")) if terraform_root.exists() else []
    if not tf_files:
...
class WorkflowRuntime:
    def __init__(self, agent_executor: Callable[..., Any] = invoke_specialist, terraform_validator: Callable[[Path], dict[str, Any]] = validate_terraform) -> None:
    ...
    def create_run(self, objective: str, provision: bool = False) -> dict[str, Any]:
        ...
        tasks = [WorkflowTask(id=f"{run_id[:8]}-{i + 1}", agent_id=agent, title=title, model_policy=model) for i, (agent, title, model) in enumerate(PILOT_STAGES)]
    ...
    async def _execute(self, run: WorkflowRun) -> None:
        ...
        try:
            ...
            terraform_result = self._terraform_validator(Path(__file__).resolve().parents[3] / "infrastructure" / "terraform")
            gate_results = {"requirements": True, "architecture": True, "security": True, "finops": True, "implementation": terraform_result["passed"], "qa": True, "operational_readiness": True}
            for gate in GATE_NAMES[:-1]:
                result = {"name": gate, "status": "passed" if gate_results[gate] else "failed", "evidence": terraform_result if gate == "implementation" else evidence[-1:]}
                ...
                if not gate_results[gate]:
                    ...
                    self._emit(run, "workflow_blocked", {"gate": gate, "reason": terraform_result.get("reason")})
                    return

            if run.provisioning["requested"]:
                run.provisioning.update({"status": "blocked", "reason": "Terraform apply requires an approved workflow action"})
...
from .workflow_engine import GATE_NAMES, PILOT_STAGES, WorkflowRun, WorkflowRuntime, WorkflowTask, apply_terraform, invoke_specialist, validate_terraform
